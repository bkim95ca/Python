...
class Party:
    ...
    def get_by_id(cls, data):
        ...
        planner = user_model.User(user_data) #planner attribute that is a user  instance
        this_party.planner = planner 
        return this_party #return a party instance with a planner attribute that is a user instance
    
    @classmethod
    def update(cls, data):
        query = "UPDATE parties SET what = %(what)s, location = %(location)s, date = %(date)s, all_ages = %(all_ages)s, description = %(description)s"\
            "WHERE id = %(id)s;"
        return connectToMySQL(DATABASE).query_db(query, data)
